...
一個應用系統的設定通常在不同的部署(./codebase)中(staging, production, developer環境等...)，會不相同。設定項目包含：

* 資料庫的處理、Memcached，以及其他 [後端服務](./backing-services) 的設定
* 外部服務的認證，如 Amazon S3、Twitter 等
* 每份部署特有的設定值，如特定主機名稱(canonical hostname)等
